...
public class ListPageUtil {
    ...
    public static List<Integer> getPageSet(int dataCount,
                                           int currentPage,
                                           int pageSize,
                                           int pageSetDisplay) {
        int pageCount = (dataCount - 1) / pageSize + getOneIfNotZero(dataCount);

        int currentPageSetIndex = (currentPage - 1) / pageSetDisplay
                + getOneIfNotZero(currentPage);
        int maxPageSet = (pageCount - 1) / pageSetDisplay
                + getOneIfNotZero(pageCount);
        int maxPageSetAndCurrentPageSetIndexDiff = maxPageSet
                - currentPageSetIndex;
        int zeroIfPageSetDiffNotPositive = convertNegativeToZero(
                maxPageSetAndCurrentPageSetIndexDiff);
        ...
        int pageCountAtLastSet = ((pageCount - 1) % pageSetDisplay + 1)
                * displayMaxPageSet;
        int abbrPageCount = Math.abs(pageSetDisplay - pageCountAtLastSet);

        int pageCountToDisplay = pageCountAtLastSet + (abbrPageCount
                * getOneIfNotZero(zeroIfPageSetDiffNotPositive));
    ...
    public static boolean isPreviousPageSetAvailable(int dataCount,
                                                     int currentPage,
                                                     int pageSize,
                                                     int pageSetDisplay) {
    ...
    public static boolean isNextPageSetAvailable(int dataCount,
                                                 int currentPage,
                                                 int pageSize,
                                                 int pageSetDisplay) {
        int pageCount = (dataCount - 1) / pageSize + getOneIfNotZero(dataCount);
